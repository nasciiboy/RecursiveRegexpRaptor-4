...
enum RE_TYPE { PATH, GROUP, HOOK, SET, BACKREF, META, RANGEAB, POINT, SIMPLE };
...
static int  matchMeta    ( const int  index, const int   chr );
...
static int trekking( const int index ){
  ...
  case COM_HOOK_INI   :
    ...
    break;
  // COM_SET_INI COM_BACKREF COM_META COM_POINT !COM_RANGEAB COM_SIMPLE
